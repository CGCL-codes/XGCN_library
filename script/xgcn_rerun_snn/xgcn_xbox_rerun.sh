...
python $PROJECT_ROOT'/'main/main.py $PROJECT_ROOT \
    ...
    --use_special_dnn 1 --dnn_arch '[torch.nn.Linear(64, 1024), torch.nn.Tanh(), torch.nn.Linear(1024, 64)]' \
...
################
SEED=1
RESULTS_DIR='xgcn/[best-without-ssnet]'
...
python $PROJECT_ROOT'/'main/main.py $PROJECT_ROOT \
    ...
    --file_test $DATA_ROOT'/test.pkl' \
    --prop_type 'lightgcn' --num_gcn_layers 1 \
    --use_special_dnn 0 --dnn_arch '[torch.nn.Linear(64, 1024), torch.nn.Tanh(), torch.nn.Linear(1024, 1024), torch.nn.Tanh(), torch.nn.Linear(1024, 64)]' \
    --renew_and_prop_freq $T --K $K --endure 3 \
    --emb_init_std 1.0 \
    >> $LOG_FILE
...
################
SEED=1
RESULTS_DIR='xgcn/[best-without-ssnet-2layer]'
...
python $PROJECT_ROOT'/'main/main.py $PROJECT_ROOT \
    ...
    --file_test $DATA_ROOT'/test.pkl' \
    --prop_type 'lightgcn' --num_gcn_layers 1 \
    --use_special_dnn 0 --dnn_arch '[torch.nn.Linear(64, 1024), torch.nn.Tanh(), torch.nn.Linear(1024, 64)]' \
    --renew_and_prop_freq $T --K $K --endure 3 \
    --emb_init_std 1.0 \
    >> $LOG_FILE
