...
def multi_pos_all_metrics(pos: list, all_target_score):
    # results_dict_list = _multi_pos_all_metrics(num_users, pos, all_target_score)
...
def _multi_pos_all_metrics(num_users, pos: list, all_item_score):
    results_dict_list = [
    ...
    for i in numba.prange(len(pos)):
        ...
        ground_truth_label = np.zeros(len(all_score))
        ground_truth_label[:len(_pos)] = 1
        
        pred_label = np.zeros(len(all_score))
        pred_label[_pos] = 1
        pred_label = pred_label[np.argsort(all_score)][::-1]
        ...
        # calc recall
        for k in topk_list:
            # results_dict['r' + str(k)] = pred_label[:k].sum() / ground_truth_label[:k].sum() <- wrong
            results_dict['r' + str(k)] = pred_label[:k].sum() / ground_truth_label.sum()
        
        # calc ndcg
        w = _get_ndcg_weights(len(pred_label))
        s = pred_label * w
        truth_s = ground_truth_label * w
        for k in topk_list:
            results_dict['n' + str(k)] = s[:k].sum() / truth_s[:k].sum()
        results_dict['ndcg'] = s.sum() / truth_s.sum()
        
        # calc mrr
        w = _get_mrr_weights(len(pred_label))
        s = pred_label * w
        truth_s = ground_truth_label * w
        results_dict['mrr'] = s.sum() / truth_s.sum()
